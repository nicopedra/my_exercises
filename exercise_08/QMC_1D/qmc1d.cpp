...
int main()
{
	...
	ifstream Para("par.txt");
	Para >> mu;
	Para >> sigma;
	Para.close();

#ifdef Const
	cout << "using a constant Psi WaveFunction" << endl;
#else
	cout << "using the trial WaveFunction with optimize parameters" << endl;
#endif
...
double variationalWaveFunction_second(double v)
{
	...
	double e1 = exp(-(v-mu)*(v-mu)/(double)(2*sigma*sigma) ) / (sigma*sigma);
        double e2 = exp(-(v+mu)*(v+mu)/(double)(2*sigma*sigma) ) / (sigma*sigma);
        double appo1 = (1.-(v-mu)*(v-mu)/(sigma*sigma));
        double appo2 = (1.-(v+mu)*(v+mu)/(sigma*sigma));
