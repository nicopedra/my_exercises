...
//reset the hystogram of g(r)
  for (int k=igofr; k<igofr+nbins; ++k) walker[k]=0.0;//igofr è l'indice per l'istogramma
...
for (int k=igofr;k<n_props;k++) {
    double deltaVr = 4.*pi/3. * ( pow((k-igofr+1)*bin_size,3) - pow((k-igofr)*bin_size,3) );
    stima_g = (blk_av[k]/double(blk_norm) ) / (rho*(double)npart*deltaVr); 
    glob_av[k] += stima_g;
    glob_av2[k] += stima_g*stima_g;
    err_press=Error(glob_av[k],glob_av2[k],iblk);

    Gofr << iblk << setw(wd) << stima_g << setw(wd) << glob_av[k]/(double)iblk << setw(wd) << err_gdir << endl;
    }
